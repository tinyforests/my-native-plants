document.addEventListener('DOMContentLoaded', function() {
    ...
    function searchEVC(lat, lon, address) {
        ...
        var buffer = 0.03;  // Increased from 0.01 to catch urban areas with sparse EVC mapping
    ...
    // ============================================================================
    // SEARCH LOGGING - IMAGE BEACON METHOD
    // ============================================================================
    function logSearchQuery(searchAddress, lat, lon, evcCode, evcName) {
        ...
        try {
            ...
            var params = new URLSearchParams({
                'entry.1872197451': timestamp,
                'entry.325945624': searchAddress || 'Unknown',
                'entry.1772337080': evcName || '',
                'entry.574020621': evcCode || '',
                'entry.895524211': lat ? lat.toFixed(6) : '',
                'entry.174755702': lon ? lon.toFixed(6) : ''
            });
            
            // Use image beacon to submit (bypasses CORS)
            var img = new Image();
            ...
            img.onerror = function() {
                console.log('✓ Search logged (form submitted)');
                // Google Forms returns error but still records the response
            };
            
            img.src = FORM_URL + '?' + params.toString();
